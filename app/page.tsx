...
const TIMELINE_MARKERS = [
  ...
  { year: 50, pct: '100%', align: 'right'  as const, label: 'COVENANT_END' },
];
...
export default function ObsidianTerminal() {
  ...
  return (
    <main className="relative w-full h-screen overflow-hidden bg-black select-none">
      ...
      {/* Clock HUD — top right */}
      <div className="fixed top-8 right-6 md:right-16 z-20 font-mono text-[10px] text-neutral-400">
      ...
      <div className="scroll-container" onScroll={handleScroll}>
        ...
        <section className="relative w-full h-screen flex flex-col justify-between bg-transparent overflow-hidden">
          ...
          <div className="absolute top-8 left-6 md:top-16 md:left-16 z-10 pt-4 md:pt-8 overflow-hidden max-w-[50%] md:max-w-none">
            <p className="font-mono text-[9px] md:text-[12px] tracking-[0.25em] md:tracking-[0.3em] text-[#00F0FF]/28 uppercase whitespace-nowrap">
        ...
        <section className="relative w-full h-screen flex flex-col justify-between bg-transparent border-t border-[rgba(255,255,255,0.02)] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 w-full h-[64%] md:h-auto md:w-[44%] md:left-[8%] md:top-1/2 md:-translate-y-1/2 flex flex-col justify-center px-6 pb-10 pt-8 md:p-0 z-10 hud-scrim-mask">
            ...
            <span className="font-mono text-[10px] tracking-[0.35em] text-[#00F0FF]/50 uppercase mt-2 mb-6">
              KPS / MONTH // BONUS_REWARD_POOL
            </span>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {[
                ...
                <span
                  key={chip}
                  className="font-mono text-[9px] tracking-[0.2em] text-[#00F0FF]/50 border border-[rgba(0,240,255,0.12)] px-2.5 py-1 uppercase whitespace-nowrap"
                >
                  {chip}
                </span>
              ))}
            ...
            <p className="mt-4 max-w-xs text-[13px] font-light tracking-wider text-neutral-400 leading-relaxed">
              Commit KPS to secure the network. Unlock randomized, VRF-verified bonus rewards — strict 29-day claim window, principal always returned.
            </p>
          </div>

          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 font-mono text-[10px] tracking-[0.25em] text-[#E5E5E5]/20 uppercase z-10 hidden md:block">
        ...
        <section className="relative w-full h-screen flex flex-col justify-between bg-transparent border-t border-[rgba(255,255,255,0.02)] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 w-full h-[60%] flex flex-col justify-center px-6 pb-10 pt-8 z-10 hud-scrim-mask md:hidden">
            ...
            <div className="mt-6 font-mono text-[9px] tracking-[0.3em] text-[#00F0FF]/35 uppercase">
              10% MONTHLY_SWEEP → PUBLIC_GOODS_VAULT
            </div>
          </div>
          ...
          {/* Right column — system status panel (desktop only) */}
          <div className="hidden md:flex absolute right-[6%] top-1/2 -translate-y-1/2 w-[32%] flex-col z-10">
            <div className="border border-[rgba(0,240,255,0.1)] bg-[rgba(0,18,28,0.6)] p-6">
              <div className="font-mono text-[9px] tracking-[0.3em] text-[#00F0FF]/40 uppercase mb-4">
                // OUTFLOW_SYSTEM_STATUS
              </div>
              {([
                ['STREAM_STATUS',   'ACTIVE'],
                ['MONTHLY_SWEEP',   '10%'],
                ['VAULT_TARGET',    'PUBLIC_GOODS_FUND'],
                ['GOVERNANCE',      'DAO_NOMINATED'],
                ['COMPLIANCE_GATE', 'KPS_LTD'],
                ['RELEASE_CYCLE',   'ANNUAL'],
              ] as [string, string][]).map(([key, val]) => (
                <div key={key} className="flex justify-between items-baseline py-2 border-b border-[rgba(255,255,255,0.04)] last:border-0">
                  <span className="font-mono text-[10px] tracking-[0.15em] text-neutral-500 uppercase">{key}</span>
                  <span className="font-mono text-[10px] tracking-[0.15em] text-[#00F0FF]/70 uppercase">{val}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="relative w-full h-screen flex flex-col justify-between bg-transparent border-t border-[rgba(255,255,255,0.02)] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 w-full h-[70%] md:h-auto md:w-[52%] md:left-[8%] md:top-1/2 md:-translate-y-1/2 flex flex-col justify-center px-6 pb-8 pt-6 md:p-0 z-10 hud-scrim-mask">
            {/* 200 allocation squares: 20 cols × 10 rows at 14px */}
              ...
              {ALLOCATION_SQUARES.map((i) => (
                <div
                  ...
                  style={{
                    ...
                    opacity: i < 18 ? 0.65 : 0.12,
                    border: i < 18 ? '1px solid rgba(0,240,255,0.4)' : '1px solid rgba(0,240,255,0.08)',
                  }}
                />
              ))}
            </div>
            <div className="font-mono text-[9px] tracking-[0.3em] text-[#00F0FF]/40 uppercase mt-3 mb-6">
              18 / 200 ALLOCATED
            </div>
            <h2
        ...
        <section className="relative w-full h-screen flex flex-col justify-between bg-transparent border-t border-[rgba(255,255,255,0.02)] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 w-full h-[76%] md:h-auto md:w-[52%] md:left-[8%] md:top-1/2 md:-translate-y-1/2 flex flex-col justify-center px-6 pb-8 pt-6 md:p-0 z-10 hud-scrim-mask">

            {/* Horizontal proportional timeline */}
            <div className="relative mb-8" style={{ height: '52px', maxWidth: '480px', width: '100%' }}>
              ...
              {TIMELINE_MARKERS.map(({ year, pct, align, label }) => (
                ...
                >
                  ...
                  <div className="font-mono text-[7px] tracking-[0.15em] mt-0.5" style={{ color: 'rgba(0,240,255,0.28)', whiteSpace: 'nowrap' }}>
                    {label}
                  </div>
                </div>
              ))}
            ...
            {/* CTAs */}
            <div className="mt-8 flex flex-col sm:flex-row items-start justify-start gap-3 w-full">
              <a
                href="https://app.krypswift.com"
                target="_blank"
                rel="noopener noreferrer"
                className="text-center border border-[rgba(0,240,255,0.3)] bg-transparent text-white px-6 py-3 font-mono text-[10px] uppercase tracking-[0.15em] transition-all duration-300 hover:border-[#00F0FF] hover:shadow-[0_0_15px_rgba(0,240,255,0.4)] w-full sm:w-auto"
              >
                Access Portal
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-center border border-[rgba(255,255,255,0.12)] bg-transparent text-neutral-400 hover:text-white px-6 py-3 font-mono text-[10px] uppercase tracking-[0.15em] transition-all duration-300 hover:border-[rgba(0,240,255,0.3)] w-full sm:w-auto"
              >
                Whitepaper v6.0
              </a>
              <a
            ...
            <div className="mt-6 flex flex-row items-center justify-start gap-2">
              <a href="https://t.me/krypswift" target="_blank" rel="noopener noreferrer" aria-label="Telegram"
                className="flex items-center justify-center w-10 h-10 border border-[rgba(255,255,255,0.12)] text-neutral-500 hover:text-[#00F0FF] hover:border-[rgba(0,240,255,0.4)] hover:shadow-[0_0_10px_rgba(0,240,255,0.2)] transition-all duration-300">
              ...
              <a href="https://x.com/krypswift" target="_blank" rel="noopener noreferrer" aria-label="X"
                className="flex items-center justify-center w-10 h-10 border border-[rgba(255,255,255,0.12)] text-neutral-500 hover:text-[#00F0FF] hover:border-[rgba(0,240,255,0.4)] hover:shadow-[0_0_10px_rgba(0,240,255,0.2)] transition-all duration-300">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </a>
              <a href="https://discord.gg/krypswift" target="_blank" rel="noopener noreferrer" aria-label="Discord"
                className="flex items-center justify-center w-10 h-10 border border-[rgba(255,255,255,0.12)] text-neutral-500 hover:text-[#00F0FF] hover:border-[rgba(0,240,255,0.4)] hover:shadow-[0_0_10px_rgba(0,240,255,0.2)] transition-all duration-300">
              ...
              {/* GitHub — placeholder until repo is public */}
            ...
            {/* Deployer wallet — Article XI compliance */}
            <div className="mt-6 font-mono text-[10px] text-neutral-600 tracking-[0.2em] uppercase">
              [ DEPLOYER: 0x000...000 ]
            </div>
